...
select 100+row_number() over(), country, city, state, region, postal_code from (select distinct country, city, state, region, postal_code from stg.orders ) a;
--data quality check
...
from 
	generate_series(date '2000-01-01', date '2030-01-01', interval '1 day') as t(date);
--checking
...
-- вставка данных из таблицы stg.people в dw.people_dim
SELECT
	*
INTO
	dw.people_dim
...
-- создание таблицы dw.returns_dim, в которой список уникальных заказов и информация по возврату
...
FROM
	(SELECT
		DISTINCT f.order_id,
		CASE 
			WHEN rd.returned1 IS NULL THEN 'No'
			ELSE 'Yes'
		END AS returned
	FROM 
		dw.sales_fact AS f
	LEFT JOIN 
			(SELECT 
				DISTINCT order_id,
				returned AS returned1
			FROM 
				stg.RETURNS) AS rd
		ON f.order_id = rd.order_id) AS m;
